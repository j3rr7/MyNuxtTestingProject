      ...
      <div class="min-h-40">
        <Transition 
          ...
          <div v-if="currentStep === 1" key="step1">
            <UForm ref="tokenFormRef" :schema="tokenSchema" :state="state" novalidate @submit="handleTokenSubmit">
              <div class="flex items-center justify-center">
                <UFormField name="token" error="Please enter a valid code">
                  <UInput 
                    v-model="state.token" placeholder="123-456" size="xl" :disabled="loading" autofocus
                    input-class="w-[240px] text-center font-mono tracking-[0.3em] text-2xl placeholder:text-slate-600"
                    name="token" inputmode="numeric" />
                </UFormField>
              </div>

              <div class="pt-6">
                <UButton 
                  type="submit" size="lg" block :loading="loading" :disabled="state.token.replace('-', '').length !== 6 || loading
                  ">
          ...
          <div v-else-if="currentStep === 2" key="step2" class="space-y-6">
            <div>
              ...
              <div class="flex items-center gap-2 p-2 bg-slate-800/50 rounded-md border border-slate-700">
                <UIcon name="i-heroicons-lock-closed-solid" class="text-green-400" />
                <p class="font-mono text-slate-300">{{ state.token }}</p>
              </div>
            </div>

            <UForm :schema="nameSchema" :state="state" class="space-y-2" @submit="handleNameSubmit">
              <div class="flex items-center justify-center">
                <UFormField label="Your Name" name="name">
                  <UInput 
                    ref="nameInputRef" v-model="state.name" name="name" placeholder="e.g., Alex" size="lg"
                    :disabled="loading"
                    input-class="w-[240px] text-center font-mono tracking-[0.3em] text-2xl placeholder:text-slate-600"
                    maxlength="50" />
                </UFormField>
          ...
          <div v-else key="step3" class="text-center p-4">
            <div class="mx-auto w-16 h-16 bg-green-500/20 rounded-full flex items-center justify-center mb-4">
              <UIcon name="i-heroicons-check-badge-solid" class="w-9 h-9 text-green-400 animate-bounce" />
            </div>
            ...
            <UButton variant="link" icon="i-heroicons-arrow-path-20-solid" class="mt-6" @click="resetForm">
              Refresh
            </UButton>
          </div>
        </Transition>
